...
const refreshStableDownload = async () => {
  ...
  try {
    ...
    // Stable releases are Developer ID-signed, notarized, and stapled. Require
    // the exact DMG contract shared by the website and in-app updater; an
    // incomplete release falls back to the releases page instead of silently
    // selecting a source archive or obsolete application ZIP.
    const asset = assets.find(candidate => candidate.name === 'whisper_hotkey.dmg');
